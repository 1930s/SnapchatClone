            ...
            if let posts = snapshot.value as? [String:AnyObject] {
                user.getReadPostIDs(completion: { (ids) in
                    for postKey in posts.keys {
                        ...
                        let dict = posts[postKey] as! [String:String]
                        
                        let post = Post(id: postKey, username: dict["username"]!, postImagePath: dict["imagePath"]!, thread: dict["thread"]!, dateString: dict["date"]!, read: ids.contains(postKey))
